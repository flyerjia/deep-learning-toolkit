# -*- encoding: utf-8 -*-
"""
@File    :   re_msie_egp_model.py
@Time    :   2023/02/02 11:13:57
@Author  :   jiangjiajia
"""
import copy
...
class REMSIEEGPModel(BaseModel):
    def __init__(self, **kwargs):
        ...
        self.criterion = MultiLabelCrossentropy(reduction='mean')
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, labels=None, label_mask=None, phase=None):
        ...
        if phase == 'train' and labels is not None and label_mask is not None:
            ...
            labels = labels[label_mask == 1].view(-1, self.num_labels)
    ...
    def get_predictions(self, forward_output, forward_target, dataset, batch_start_index=0):
        ...
        for each_output in forward_output['logits']:
            ...
            head2head_set = set()
            tail2tail_set = set()
            for i, j, z in zip(*np.where(each_output > 0)):
                if i == j:
                    continue
                start_index = i - 1
                end_index = j - 1
                label_type, label = dataset.id2label[z].split('_')
                if label_type == 'EH2ET':
