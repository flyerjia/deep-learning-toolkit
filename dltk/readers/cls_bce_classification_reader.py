# -*- encoding: utf-8 -*-
"""
@File    :   cls_bce_classification_reader.py
@Time    :   2022/09/05 15:22:20
@Author  :   jiangjiajia
"""
import torch
...
class CLSClassificationReader(BaseReader):
    ...
    def convert_item(self, each_data):
        ...
        label_str = each_data.get('label', -1)
        ...
        label = torch.zeros(len(self.label2id))
        if label_str != -1:
            labels = label_str.split(',')
            for temp_label in labels:
                if temp_label in self.label2id.keys():
                    label[self.label2id[temp_label]] = 1.0
        return {
            'input_ids': input_ids,
    ...
    def collate_fn(self, batch_data):
        ...
        labels = torch.vstack([each_data['labels'] for each_data in batch_data])
        return {
            'input_ids': input_ids,
            'token_type_ids': token_type_ids,
            'attention_mask': attention_mask,
