# -*- encoding: utf-8 -*-
"""
@File    :   ner_efficient_globalpointer_reader.py
@Time    :   2023/01/29 17:29:37
@Author  :   jiangjiajia
"""
import torch
...
class NEREfficientGlobalPointerReader(BaseReader):
    def __init__(self, phase, data, config):
        super(NEREfficientGlobalPointerReader, self).__init__(phase, data, config)
    ...
    def convert_item(self, each_data):
        ...
        labels = torch.zeros((self.config['max_seq_len'], self.config['max_seq_len'], self.num_labels)).long()
        ...
        for entity in entities:
            ...
            if start_idx < self.config['max_seq_len'] - 2 and end_idx < self.config['max_seq_len'] - 2:
                labels[start_idx + 1, end_idx + 1, self.label2id[type]] = 1  # 增加CLS位置
        return {
            'input_ids': input_ids,
...
reader = NEREfficientGlobalPointerReader
