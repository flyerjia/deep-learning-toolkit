# -*- encoding: utf-8 -*-
"""
@File    :   re_msie_egp_reader.py
@Time    :   2023/02/01 17:06:08
@Author  :   jiangjiajia
"""
import torch
...
class REMSIEEGPReader(BaseReader):
    def __init__(self, phase, data, config):
        super(REMSIEEGPReader, self).__init__(phase, data, config)
    ...
    def convert_item(self, each_data):
        ...
        for each_spo in spo_list:
            ...
            if subject_start_idx < self.config['max_seq_len'] - 2 and subject_end_idx < self.config['max_seq_len'] - 2 and \
                ...
                labels[subject_start_idx + 1, subject_end_idx + 1, self.label2id['EH2ET_' + subject_type]] = 1
                labels[object_start_idx + 1, object_end_idx + 1, self.label2id['EH2ET_' + object_type]] = 1
                labels[subject_start_idx + 1, object_start_idx + 1, self.label2id['H2H_' + predicate]] = 1
                labels[subject_end_idx + 1, object_end_idx + 1, self.label2id['T2T_' + predicate]] = 1
        return {
            'input_ids': input_ids,
