# -*- encoding: utf-8 -*-
"""
@File    :   multi_span_relation_extraction_reader_ce.py
@Time    :   2022/08/12 10:39:09
@Author  :   jiangjiajia
"""
import torch
    ...
    def convert_item(self, each_data):
        ...
        for relation_of_mention in relation_of_mention_list:
            ...
            if relation in [1, 3]:
                ...
                labels[head_start_idx, head_end_idx] = 1
                labels[tail_start_idx, tail_end_idx] = 1
                # s->o
                labels[head_start_idx, tail_start_idx] = self.label2id['H2H_' + str(relation)]
                labels[head_end_idx,  tail_end_idx] = self.label2id['T2T_' + str(relation)]
            else:  # 2
                head_start_idx, head_end_idx = head['start_idx'] + 1, head['end_idx']
                ...
                labels[head_start_idx, head_end_idx] = 1
                labels[sub_head_start_idx, sub_head_end_idx] = 1
                labels[sub_tail_start_idx, sub_tail_end_idx] = 1
                # s->o
                labels[sub_head_start_idx, sub_tail_start_idx] = self.label2id['H2H_' + str(sub_relation)]
                labels[sub_head_end_idx,  sub_tail_end_idx] = self.label2id['T2T_' + str(sub_relation)]
                # c->s
                labels[head_start_idx, sub_head_start_idx] = self.label2id['H2H_' + str(relation)]
                labels[head_end_idx, sub_head_end_idx] = self.label2id['T2T_' + str(relation)]
                # c->o
                labels[head_start_idx, sub_tail_start_idx] = self.label2id['H2H_' + str(relation)]
                labels[head_end_idx, sub_tail_end_idx] = self.label2id['T2T_' + str(relation)]
        return {
            'input_ids': input_ids,
